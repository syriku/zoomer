...
constructor WindowsWorkspaceWindow;
begin
...
method WindowsWorkspaceWindow.setCapturedImage(source: BitmapSource);
begin
...
method WindowsWorkspaceWindow.showOnDisplay(display: WorkspaceDisplay);
var
  bounds: WindowsRect;
begin
...
method WindowsWorkspaceWindow.renderTransform(transform: WorkspaceTransform) showHud(showHud: Boolean);
begin
...
method WindowsWorkspaceWindow.closeForDismissal;
begin
  if fIsClosed then
    exit;
...
method WindowsWorkspaceWindow.imageSizeChanged(sender: Object) eventArgs(eventArgs: SizeChangedEventArgs);
begin
  applyTransform;
end;

method WindowsWorkspaceWindow.restoreHighQuality(sender: Object) eventArgs(eventArgs: EventArgs);
begin
  fQualityTimer.Stop;
  RenderOptions.SetBitmapScalingMode(fImage, BitmapScalingMode.HighQuality);
end;

method WindowsWorkspaceWindow.fadeHud(sender: Object) eventArgs(eventArgs: EventArgs);
begin
  fHudDelayTimer.Stop;
...
method WindowsWorkspaceWindow.applyTransform;
begin
...
method WindowsWorkspaceWindow.showHudForScale(scale: Double);
begin
...
method WindowsWorkspaceWindow.OnMouseLeftButtonDown(eventArgs: MouseButtonEventArgs);
begin
...
method WindowsWorkspaceWindow.OnMouseMove(eventArgs: MouseEventArgs);
begin
...
method WindowsWorkspaceWindow.OnMouseLeftButtonUp(eventArgs: MouseButtonEventArgs);
begin
...
method WindowsWorkspaceWindow.endDrag;
begin
  if not fIsDragging then
    exit;
  fIsDragging := false;
...
method WindowsWorkspaceWindow.OnPreviewMouseWheel(eventArgs: MouseWheelEventArgs);
begin
...
method WindowsWorkspaceWindow.OnPreviewKeyDown(eventArgs: KeyEventArgs);
begin
...
method WindowsWorkspaceWindow.tryHandlePresetScale(eventArgs: KeyEventArgs): Boolean;
begin
...
method WindowsWorkspaceWindow.hasNoShortcutModifiers: Boolean;
begin
  result := Keyboard.Modifiers = ModifierKeys.None;
end;

method WindowsWorkspaceWindow.OnClosing(eventArgs: CancelEventArgs);
begin
...
method WindowsWorkspaceWindow.requestCommand(command: WorkspaceCommand);
begin
...
method WindowsWorkspaceWindow.requestDismissal;
begin
...
method WindowsWorkspaceWindow.windowMessageHook(windowHandle: IntPtr) message(message: Int32) wParam(wParam: IntPtr) lParam(lParam: IntPtr) handled(var handled: Boolean): IntPtr;
begin
