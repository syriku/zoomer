...
`MacApp` 负责 macOS 专属能力，并吸收原 `native/Zoomer.Native` 库的职责。当前已接入菜单栏入口、屏幕录制权限检查、鼠标所在显示器选择、ScreenCaptureKit 截图、AppKit 全屏工作窗口和基础缩放/平移/翻转输入：

- 应用生命周期、菜单与后续的 `⌥⌘Z` 全局快捷键；
...
3. 完成 macOS 全局快捷键、指针管理、激光笔、聚光、绘图与其余输入细节。
4. 验证截图资源所有权、线程调度、输入差异与权限失败路径。
