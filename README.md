...
本分支正在将 `main` 的 .NET 10/C# 实现重构为 RemObjects Elements 项目，源码语言统一使用 Oxygene（`.pas`）。第一阶段已建立 `Shared` 的工作区状态与变换逻辑，以及 `MacApp` 的 Cocoa/ScreenCaptureKit 实现；`WPFApplication` 仍保持模板状态。它尚不能替代 `main` 中可运行的完整应用，后续工作会按既定边界补齐交互与平台能力。
...
2. 为 Shared 增加 Echoes 目标和 Oxygene 测试项目，验证状态机与变换行为。
