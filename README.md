...
- 按 `D` 切换激光笔绘图模式；开启后左键拖动保留轨迹，轨迹会随画布缩放、平移和翻转，松开左键 3 秒后渐隐消失；再按 `D` 恢复左键拖动平移
- 按 `M` 水平翻转截图，再按一次恢复
- 按 `0` 将缩放恢复到 1 倍但保留图片位置（也支持 `⌘0`）；按 `C` 将图片居中但保留缩放；按 `R` 完全重置缩放、位置和翻转；按 `1`、`2`、`9` 分别切换到 1.5、2、0.7 倍；Esc 退出工作模式
...
- 按 `D` 切换激光笔绘图模式；开启后左键拖动保留轨迹，轨迹会随画布缩放、平移和翻转，松开左键 3 秒后渐隐消失；再按 `D` 恢复左键拖动平移
- 按 `M` 水平翻转截图，再按一次恢复
- 按 `0` 将缩放恢复到 1 倍但保留图片位置（也支持 `Ctrl+0`）；按 `C` 将图片居中但保留缩放；按 `R` 完全重置缩放、位置和翻转；按 `1`、`2`、`9` 分别切换到 1.5、2、0.7 倍；Esc 退出工作模式
